# Codes in this files are from SincNet https://github.com/mravanelli/SincNet
...
class SincNet_global_2(nn.Module):
    
    def __init__(self,input_dim,fs):
       super(SincNet_global_2,self).__init__()
       ...
       for i in range(self.N_cnn_lay):
         ...
         if i==1:
          self.out_filt_len = current_input 
       self.out_dim=int(self.cnn_N_filt[i-1])
    ...
    def forward(self, x):
       ...
       for i in range(self.N_cnn_lay-1):
...
class GRU_MLP(nn.Module):
    def __init__(self, inp):
      ...
      self.gru = nn.GRU(inp, 8, 1, batch_first=True)
      self.act_1 = act_fun('leaky_relu')
    ...
    def forward(self, x,h):
      ...
      x,h = self.gru(x,h)
      self.before_act = x
      x=x[:,-1,:] #get last time step of GRU
      x = self.bn_1(x)
      x = self.act_1(x)
      x = self.FC(x)
      x = self.sigmoid(x) 
